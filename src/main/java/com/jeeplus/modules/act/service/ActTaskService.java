...
public class ActTaskService extends BaseService {
	...
	public List<HashMap<String,String>> todoList(Act act){
		...
		if (act.getAppNo() != null && !"".equals(act.getAppNo())){
			todoTaskQuery.processInstanceBusinessKeyLike("%"+act.getAppNo() + "%");
		}
		...
		for (Task task : todoList) {
			...
			map.put("task.description", task.getDescription());
		...
		if (act.getAppNo() != null  && !"".equals(act.getAppNo())){
			toClaimQuery.processInstanceBusinessKeyLike(act.getAppNo());
		}
	...
	public Page<HashMap<String,String>> historicList(Page<HashMap<String,String>> page, Act act){
		...
		if (act.getAppNo() != null && !"".equals(act.getAppNo())){
			histTaskQuery.processInstanceBusinessKeyLike("%"+act.getAppNo() + "%");
		}
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	...
	@SuppressWarnings("unused")
	@Transactional(readOnly = false)
	public String startProcessB(String procDefKey, String businessTable, String businessId,String businessKey, String title, Map<String, Object> vars) {
		//String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId())		
		String userId = (String) vars.get("applyUserId");
		if(userId == null ){
			 userId = UserUtils.getUser().getLoginName();
...
//	}
	 ...
	 * 获取已办任务
	 */
	public HistoricTaskInstance getByHisTask(String taskId){
		HistoricTaskInstance tas =  historyService.createHistoricTaskInstanceQuery().taskId(taskId).singleResult();
		return tas;
	}
	/**
	 * 获取当前步骤流程信息
	 */
	public Map<String,Object> getThisTask(String procInstId){
	//	HistoricTaskInstance tas =  historyService.createHistoricTaskInstanceQuery().processInstanceId(procInstId).taskAssignee(UserUtils.getUser().getLoginName()).singleResult();
		//Task task = taskService.createTaskQuery().processInstanceId(procInstId).taskAssignee(UserUtils.getUser().getLoginName()).singleResult();
		//执行实例  
        
        
        Task task =  taskService.createTaskQuery().taskAssignee(UserUtils.getUser().getLoginName()).processInstanceId(procInstId).includeProcessVariables().singleResult();
        Map ds = task.getProcessVariables();
        Map<String,Object> map = new HashMap<String, Object>();
        map.put("processName", ProcessDefCache.get(task.getProcessDefinitionId()).getName());
        map.put("appNo",task.getProcessVariables().get("appNo"));
