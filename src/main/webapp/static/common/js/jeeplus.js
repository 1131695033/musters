...
(function() {
    ...
    //打开对话框(添加修改)
	 ...
	 //打开对话框(添加修改)
	   openDialogA: function(title,url,width,height, $table){
		var auto = true;//是否使用响应式，使用百分比时，应设置为false
		if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
			auto =false;
		}
	   	top.layer.open({
	   		...
	   		btn2: function(index, layero){
	   		    //按钮【按钮】的回调
	   			top.layer.confirm('你还没保存数据呢，确定要关闭么？', {
		   			btn: ['确定','取消'] //按钮
		   		}, function(){
		   			top.layer.closeAll();
		   		}, function(){
	   ...
	   /**表具选择框**/
	   openGmInfoSelectDialog:function(isMultiSelect,meterBore, yesFuc){
		   if(meterBore == null || meterBore == undefined){
			   meterBore = "";
		   }
		   top.layer.open({
			    type: 2, 
			    ...
			    auto:true,
		        maxmin: true, //开启最大化最小化按钮
			    content: ctx+"/dgminfo/dGmInfo/dGmInfoSelect?isMultiSelect="+isMultiSelect+"&meterBore="+meterBore,
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
			    	var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
			    	var names = layero.find("iframe")[0].contentWindow.getNameSelections();
					if(ids.length ==0){
						jp.warning("请选择至少一个表具!");
						return;
					}
			    	// 执行保存
			    	yesFuc(ids.join(","), names.join(","));
	   ...
	   /**抄表册选择框**/
	   openSectSelectDialog:function(isMultiSelect, yesFuc){
		   top.layer.open({
			    type: 2, 
			    area: ['800px', '600px'],
			    title:"选择抄表册",
			    auto:true,
		        maxmin: true, //开启最大化最小化按钮
			    content: ctx+"/sect/sect/sectSelect?isMultiSelect="+isMultiSelect,
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
			    	var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
			    	var names = layero.find("iframe")[0].contentWindow.getNameSelections();
					if(ids.length ==0){
						jp.warning("请选择至少一个抄表册!");
						return;
					}
			    	// 执行保存
			    	yesFuc(ids.join(","), names.join(","));
	   ...
	   /**水价码选择框**/
	   openPPrcSelectDialog:function(isMultiSelect, yesFuc){
		   top.layer.open({
			    type: 2, 
			    area: ['800px', '600px'],
			    title:"选择水价码",
			    auto:true,
		        maxmin: true, //开启最大化最小化按钮
			    content: ctx+"/pprc/pPrc/pprcSelect?isMultiSelect="+isMultiSelect,
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
			    	var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
			    	var names = layero.find("iframe")[0].contentWindow.getNameSelections();
					if(ids.length ==0){
						jp.warning("请选择至少一个水价码!");
						return;
					}
			    	// 执行保存
			    	yesFuc(ids.join(","), names.join(","));
	   ...
	   /**户号选择框**/
	   openConsSelectDialog:function(isMultiSelect, yesFuc){
		   top.layer.open({
			    type: 2, 
			    area: ['1200px', '660px'],
			    title:"选择户号",
			    auto:true,
		        maxmin: true, //开启最大化最小化按钮
			    content: ctx+"/ccons/cCons/consSelect?isMultiSelect="+isMultiSelect,
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
			    	var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
			    	var names = layero.find("iframe")[0].contentWindow.getNameSelections();
					if(ids.length ==0){
						jp.warning("请选择至少一个户号!");
						return;
					}
			    	// 执行保存
			    	yesFuc(ids.join(","), names.join(","));
	   ...
	   /**计算公式选择框**/
	   openCalcFormulaSelectDialog:function(isMultiSelect, yesFuc){
		   top.layer.open({
			    type: 2, 
			    area: ['1200px', '660px'],
			    title:"选择计算公式",
			    auto:true,
		        maxmin: true, //开启最大化最小化按钮
			    content: ctx+"/calcformula/calcFormula/calcFormulaSelect?isMultiSelect="+isMultiSelect,
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
			    	var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
			    	var names = layero.find("iframe")[0].contentWindow.getNameSelections();
					if(ids.length ==0){
						jp.warning("请选择至少一个计算公式!");
						return;
					}
			    	// 执行保存
			    	yesFuc(ids.join(","), names.join(","));
	   ...
	   /**行业类别选择框**/
	   openiTradeCodeSelectDialog:function(isMultiSelect, yesFuc){
		   top.layer.open({
			    type: 2, 
			    area: ['1100px', '450px'],
			    title:"选择水表",
			    auto:true,
		        maxmin: true, //开启最大化最小化按钮
			    content: ctx+"/itradecode/iTradeCode/iTradeCodeSelect?isMultiSelect="+isMultiSelect,
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
			    	var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
			    	var names = layero.find("iframe")[0].contentWindow.getNameSelections();
	   ...
	   /**公共选择框**/
	   openPubSelectDialog:function(isMultiSelect,url, msg,yesFuc){
		   top.layer.open({
			    type: 2, 
			    area: ['900px', '600px'],
			    title:"选择"+msg,
			    auto:true,
		        maxmin: true, //开启最大化最小化按钮
			    content: ctx+url + "&isMultiSelect="+isMultiSelect,
			    btn: ['确定', '关闭'],
			    yes: function(index, layero){
			    	var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
			    	var names = layero.find("iframe")[0].contentWindow.getNameSelections();
					if(ids.length ==0){
						jp.warning("请选择至少一个"+msg+"!");
						return;
					}
			    	// 执行保存
			    	yesFuc(ids.join(","), names.join(","));
