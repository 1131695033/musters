...
$(document).ready(function() {
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该订单明细信息记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/order/orders/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/order/orders/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该订单明细信息记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/order/orders/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#orderTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
   ...
   function add(){
	  jp.openDialog('新增订单明细信息', "${ctx}/order/orders/form",'800px', '500px', $('#orderTable'));
  }
  function edit(id){//没有权限时，不显示确定按钮
	  ...
	  jp.openDialog('编辑订单明细信息', "${ctx}/order/orders/form?id=" + id,'800px', '500px', $('#orderTable'));
	   </shiro:hasPermission>
	  <shiro:lacksPermission name="order:orders:edit">
	  jp.openDialogView('查看订单明细信息', "${ctx}/order/orders/form?id=" + id,'800px', '500px', $('#orderTable'));
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/order/orders/detail?id="+row.id, function(testDataMainDialog){
    	var testDataMainDialogChild1RowIdx = 0, testDataMainDialogChild1Tpl = $("#testDataMainDialogChild1Tpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
		var data1 =  testDataMainDialog.orderDetailsList;
