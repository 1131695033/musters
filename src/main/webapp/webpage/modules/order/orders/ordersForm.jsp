		...
		$(document).ready(function() {
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/order/orders/save",$('#inputForm').serialize(),function(data){
						if(data.success){
...
<body class="bg-white">
		<form:form id="inputForm" modelAttribute="orders" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered">
		   <tbody>
				...
				<tr>
					<td class="width-15 active"><label class="pull-right">区域id：</label></td>
					<td class="width-35">
						<form:input path="regionId" htmlEscape="false"    class="form-control "/>
					</td>
					<td class="width-15 active"><label class="pull-right">流程类别：</label></td>
					<td class="width-35">
						<form:input path="category" htmlEscape="false"    class="form-control "/>
					</td>
				</tr>
					<tr>
					<td class="width-15 active"><label class="pull-right">当前流程节点：</label></td>
					<td class="width-35">
						<form:input path="node" htmlEscape="false"    class="form-control "/>
					</td>
					<td class="width-15 active"><label class="pull-right">地址id：</label></td>
					<td class="width-35">
						<form:input path="addressId" htmlEscape="false"    class="form-control "/>
					</td>
				</tr>
					<tr>
					...
					<td class="width-35">
						<form:input path="paymentstatus" htmlEscape="false"    class="form-control "/>
					</td>
				</tr>
		 	</tbody>
		</table>
