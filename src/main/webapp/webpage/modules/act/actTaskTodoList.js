   			...
   			,{
		        ...
		        formatter: function operateFormatter(value, row, index) {
                	if(row['task.assignee'] == undefined || row['task.assignee'] == ""){
                		 return '<a href="javascript:claim(\''+row['task.id']+'\');">'+value+'</a>';
                		 //签收任务
					}else{
						return'<a href="${ctx}/act/task/form?taskId='+row['task.id']+'&taskName='+encodeURI(row['task.name'])+'&taskDefKey='+row['task.taskDefinitionKey']+'&procInsId='+row['task.processInstanceId']+'&procDefId='+row['task.processDefinitionId']+'&status='+row.status+'">'+value+'</a>';
						//任务办理
